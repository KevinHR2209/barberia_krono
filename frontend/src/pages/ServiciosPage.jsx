    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 mt-1">{servicios.length} servicios disponibles</p>
        </div>
        ...
        {servicios.map(s => (
          <div key={s.id} className="card hover:shadow-md transition-shadow">
            <div className="flex items-start justify-between">
              <h3 className="text-gray-800 font-semibold">{s.nombre}</h3>
              <button onClick={() => eliminar(s.id)} className="p-2 bg-dark-700 hover:bg-red-50 text-gray-400 hover:text-red-500 rounded-lg transition-all border border-dark-600">
                <FiTrash2 />
              </button>
            </div>
            {s.descripcion && <p className="text-gray-500 text-sm mt-2">{s.descripcion}</p>}
            <div className="flex items-center gap-4 mt-3">
              <span className="flex items-center gap-1 text-gray-500 text-sm"><FiClock className="text-xs"/> {s.duracion_minutos} min</span>
              <span className="flex items-center gap-1 text-krono-600 font-semibold text-sm"><FiDollarSign className="text-xs"/> ${Number(s.precio).toLocaleString('es-CL')}</span>
            </div>
          </div>
        ))}
      </div>
