  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-bold text-white">Sillas</h1>
          <p className="text-gray-400 mt-1">{sillas.length} sillas en la barbería</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        {sillas.map(s => (
          <div key={s.id} className={`card border-2 ${ s.activa ? 'border-krono-500/40' : 'border-dark-500 opacity-60'}`}>
            <div className="flex items-start justify-between">
              <div className={`w-14 h-14 rounded-xl flex items-center justify-center text-2xl font-bold ${ s.activa ? 'bg-krono-500/20 text-krono-400' : 'bg-dark-600 text-gray-500'}`}>
                #{s.numero}
              </div>
              <button onClick={() => eliminar(s.id)} className="p-2 hover:bg-red-900/30 text-gray-500 hover:text-red-400 rounded-lg transition-all">
                <FiTrash2 />
              </button>
            </div>
            {s.descripcion && <p className="text-gray-400 text-sm mt-3">{s.descripcion}</p>}
            <button
              onClick={() => toggleActiva(s)}
              className={`mt-3 w-full py-1.5 rounded-lg text-xs font-medium transition-all ${
                s.activa ? 'bg-green-900/30 text-green-400 hover:bg-green-900/50' : 'bg-dark-600 text-gray-500 hover:bg-dark-500'
              }`}
            >
