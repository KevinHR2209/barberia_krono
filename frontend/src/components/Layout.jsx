...
import { FiScissors, FiCalendar, FiUsers, FiList, FiGrid, FiHome, FiClock, FiExternalLink } from 'react-icons/fi'
...
export default function Layout() {
  return (
    <div className="flex h-screen overflow-hidden bg-dark-900">
      {/* Sidebar */}
      <aside className="w-64 bg-krono-900 flex flex-col shadow-xl">
        {/* Logo */}
        <div className="p-6 border-b border-krono-800">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 bg-white/10 backdrop-blur rounded-xl flex items-center justify-center shadow-lg">
              <svg viewBox="0 0 40 40" className="w-7 h-7" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <circle cx="20" cy="14" r="1.5" fill="#93c5fd"/>
              </svg>
            </div>
            <div>
              <h1 className="text-white font-black text-xl leading-none tracking-tight">KRONO</h1>
              <p className="text-krono-300 text-xs font-medium tracking-widest uppercase">Barbería</p>
            </div>
          </div>
        </div>
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-all duration-200 ${
                  isActive
                    ...
                    : 'text-krono-200 hover:bg-white/10 hover:text-white'
                }`
              }
            >
        ...
        <div className="p-4 border-t border-krono-800">
          <a
            href="/reservar"
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-2 px-4 py-3 rounded-xl bg-white/10 hover:bg-white/20 text-white transition-all text-sm font-medium"
          >
            <FiExternalLink />
            Portal de Reservas
          </a>
          <p className="text-xs text-krono-400 text-center mt-3">© 2026 Barbería Krono</p>
        </div>
      </aside>
